...
The `{golem}` package is a framework for building production-grade Shiny Application. 
...
In other words, most of the time you will not interact with it directly, but through wrappers from `{golem}` and `{usethis}` which are listed in the `dev` scripts.
...
By default, `{golem}` uses a `fluidPage()`, which is `{shiny}` most commonly used template. 
...
It is based on the [`{config}`](https://github.com/rstudio/config) format, and allows you to define contexts, with values associated with these specific contexts. 
