...
Why?
Here is the general idea: let's say the schema in Figure \@ref(fig:14-when-optimize-1) represents your software, and its goal is to make things travel from *X1* to *X2*, but you have a bottleneck at μ.
...
> The Art of Unix Programming, [@ericraymond2003]
...
So, to sum up, there are three ways to optimize your application and R code, and the bad news is that you can not optimize for all of them:
...
Long story short, what makes the app run is not the function itself, but the printing of the function, so the object returned by `run_app()` itself can not be profiled.
