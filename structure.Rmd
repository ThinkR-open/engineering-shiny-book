...
Understanding all of this is not the purpose of this chapter: just remember that Shiny inputs and outputs id __have__ to be unique, so that the browser knows where to put what it receives from R, and R knows what to listen to from the browser. 
...
R itself has a system for namespaces ; that's what packages do and why you can have `purrr::flatten` and `jsonlite::flatten`: the function name is the same, but the two live in a different namespace, and they can mean two different things, as the symbol is evaluated inside two different namespaces. 
...
So, that's what modules are made for: creating small namespaces where you can safely define `ids` without conflicting. 
...
If you've been developing R packages, and you always try to split your functions into series of smaller functions, that's the exact same thing: you are, with just a little bit of tweaking, doing the same thing. 
...
This is a typical case of where you should reuse modules: if two or more parts are relatively similar, it's easier to bundle it inside a reusable module, and condition the ui/server with function arguments. 

![](img/tidytuesdayapp.png)

Here, are some example of how it works in the UI:
...
There are at least two approach: returning reactive, or the "stratégie du petit r" (to pronounce with a french accent). 

#### Returning values from the module

One common approach is return reactive values from one module, and to pass it to another. 
