...
+ From the developer team point of view, this steps also involves getting a clear idea of what the client is asking, in other word it involves translating the requirements to technical specifications. 
...
We have all been in a situation during a project where we tell ourselves: "I wish I had know this sooner": working on designing the application before building it helps lowering the chances for this kind of bad surprises. 
...
This part is included in the design part because it is something that you might want to think about from the very beginning: for example, some company have pre-existing `{shiny}` templates, they might want to include specific fonts, logo, icons...
...
In this step, we will cover: adding dependencies, organizing modules and functions, adding tests, documenting , and using the internal `dev` functions from `{golem}`.
...
+ In the _Version Control_ chapter, you will be introduced to `git` and to automated test using Continous Integration platforms like Travis CI or GitHub Actions. 
