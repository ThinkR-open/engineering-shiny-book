...
How does it work?
Let's make a brief parallel with the human brain, and imagine that you know that you will need to use a phone number many time in the day, and for the purpose of this thought experiment you are completely unable to remember it [^optimizing-shiny-code-1] .
...
[^optimizing-shiny-code-1]: Anyway, now that we all have smartphone, who still remembers phone numbers?
...
The downside being that you only have limited space on your screen: when your screen is covered by sticky notes, you can not store any more notes [^optimizing-shiny-code-2] .
...
[^optimizing-shiny-code-2]: In that case, you can either hide pre-existing sticky notes, or buy a bigger screen.
    ...
    If you are interesting in reading more about caching theory, we suggest the excellent *Algorithms to Live By*, by Brian Christian and Tom Griffiths [@brianchristian2016].
...
Managing cache at a system level is a very vast, fascinating topic that we can not cover here, but note that the most commonly accepted rule for deleting cache is called **LRU**, for **Least Recently Used**.
...
Natively, in `{shiny}`, if *user1* comes and launches a 15 second computation, then *user2* has to wait for this computation to finish, before launching their own 15 second computation, and *user3* has to wait the 15 seconds of *user1* plus the 15 seconds for user, etc.
...
#### B. Inner-session Asynchronousity {.unnumbered}
...
For an example of an application built using `{promise}` and `{future}`, feel free to browse [engineering-shiny.org/shinyfuture/](https://engineering-shiny.org/shinyfuture/): you'll find here an example of blocking and non-blocking processes.
