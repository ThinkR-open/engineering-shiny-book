...
That way, you can for example send a notification to the user if the process fails, either using `{shiny}` notification function, an external package like `{shinyalert}` [@R-shinyalert], or a custom JavaScript alert like [notify.js](https://github.com/ColinFay/notifyjsexample).
