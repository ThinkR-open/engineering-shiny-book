...
Let us state the truth: no matter how complex and innovative your backend is, your application is bad if your User Experience (UX) is bad. 
That is the hard truth. 
We have a natural tendency, as R-coders, to be focused on the back-end, _i.e_ the server part of the application^[aka what happens in the server side of a Shiny App]. 
Which is perfectly normal.
Chances are that you did not come to R to design front-ends^[Front-end are the visual parts of your application, the one your user interacts with, as opposed to the back-end, which is what is installed on the server, the part the end user does not see. 
In Shiny, front-end is what correspond to the UI, back-end to the server.]. 
However, if people can not understand how to use it, your application is not successful, no matter how incredible the backend is. 
...
This natural taste for back-end / server logic can deserve you on the long run: by neglecting the UI and the UX, you will make your application less likely to be adopted among your users. 
...
Aiming for simplicity is a hard thing, but some rules will help you start on a solid ground for a better user experience, thus a successful application.

There are two main contexts where you will be building a web application with R: one for professional use (_i.e_ people will rely on this app to do their job), or for fun (_i.e_ people will only use the app as a distraction). 
In both cases, people will want the app to be usable, and __easily__ usable. 
...
Think about all the professional applications and softwares that you ranted about during your professional life, all these cranky user interfaces you never understand and/or you need to relearn every time you use them. 
You do not want your app to be one of these applications. 

On the other end, if users open your app for fun, they do not fight until submission: they will give up if it is too complex to be used.
...
In this section, we will review two general principals: the "do not make me think" principle, which states that interfaces should be as self-explanatory as possible, and the "Rule of least surprise", stating that elements should behave the way they are commonly expected to behave.
These two rules aim at solving one issue: the bigger the cognitive load of your app, the harder for the end-user to use it on a daily basis.
And of course, the less successful your project. 
...
`r right_link("Don't make me think - Steve Krug", "http://sensible.com/dmmt.html")`

For example, let us have a look at the user interface of `hexmake`, a Shiny app for building hex stickers, available at https://connect.thinkr.fr/hexmake/
...
That way, you can for example send a notification to the user if the process fails, either using `{shiny}` notification function, an external package like `{shinyalert}`, or a custom JavaScript alert like [notify.js](https://github.com/ColinFay/notifyjsexample).
...
Let us for example get back to our `{tidytuesday201942}` application available at [connect.thinkr.fr/tidytuesday201942](https://connect.thinkr.fr/tidytuesday201942/)
...
Let us exemplify this with the "Render" button from the [`{tidytuesday201942}`](https://connect.thinkr.fr/tidytuesday201942/) application. 
...
Least surprise is crucial to make the user experience a good one: users rarely think that if something is behaving unexpectedly on an app, it is because of the app?
They will usually think it is their fault.
...
> When users are astonished they usually assume that they have made a mistake; they are unlikely to realize that the page has astonished them. They are more likely to feel that they are at fault for not anticipating the page. Do not take advantage of this; making users feel stupid is not endearing.
...
Let us compare `{tidytuesday201942}` to `{hexmake}` — one has a clear progression, `{hexmake}`, and has been designed as such: the upper menus design the stickers, and then once they are filled you can download them. 
...
Let us refactor our app from before:
...
But let us focus on a smaller scope, and think about some things that can be thought of as feature-creeping your Shiny app.
...
Even without color-blindness, it is already way more readable.
But let us now use the `{dichromat}` package to simulate color blindness.
