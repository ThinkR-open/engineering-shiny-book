...
Before starting to prototype and build anything, initialize a `{golem}` project! 
This will help you put your application on solid ground, and once the project is ready to be filled, you can start prototyping right inside it!

The general workflow for "prototype and build" is the following: the project manager sets up a `{golem}` project, where the first steps are filled, the general structure, with Shiny module, is set, and then the project is registered to the version control system. 
...
Once you've got that, a new project will be launched. 
...
golem::fill_desc(
  pkg_name = "ipsumapp", # The Name of the package containing the App 
...
Then, call the `golem::set_golem_options()` function, it will add information to the `golem-config.yml` file, and notably set the `{here}` package root sentinel. 
...
```{r 09-step-by-step-prototype-7, eval = FALSE}
...
```{r 09-step-by-step-prototype-15, eval = FALSE}
golem::add_js_handler( "handlers" )
```

```{r 09-step-by-step-prototype-16, include = FALSE}
...
```{r 09-step-by-step-prototype-17, eval = FALSE}
golem::add_css_file( "custom" )
```

```{r 09-step-by-step-prototype-18, include = FALSE}
