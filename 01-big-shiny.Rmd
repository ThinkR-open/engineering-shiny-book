...
Once the app is out, it is successful if it can **exist in the long run, with all the hazards that this implies**: new package versions that could potentially break the code base, sudden calls for the implementation of new features in the global interface, changing key features of the UI or the back-end, not to mention passing the code base along to someone who has not worked on the first version, and who is now in charge of developing the next version [^big-shiny-12] .
