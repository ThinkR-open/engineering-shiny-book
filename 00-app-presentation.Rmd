...
> Eric Nantz 
> Host of the R-Podcast and the Shiny Developer Series
...
It's been built by Colin, and it serves two main purposes: it helps the creation of a logo, but mainly it serves as an example of some complex features you can use inside a `{shiny}` application (image manipulation, custom CSS, linking to an external database, save and restore, etc.).
...
-   `{graysacle}` is an application built by Cervan using an external html template, available at [engineering-shiny.org/graysacle/](https://engineering-shiny.org/graysacle/).
