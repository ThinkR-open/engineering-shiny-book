...
In its core, Shiny runs R code. 
So to be efficient, the R code computing your values and returning results also has to be optimized.

Optimizing R code is such a broad topic that it would be possible to write a full book about it, and a lot of books about R already cover this topic. 
Instead of re-writting these books, we'll try to point to some crucial resources you can refer to if you want get started optimizing your R code. 
...
+ `rv <- reactiveValues` creates a `reactiveValue()` that will contains the `NULL`, and which will serve  the content of `renderPrint()` when the `future` is resolved. 
It is initiated as `NULL` so that the `renderPrint()` is silent at launch.

+ `%...>% rv() %...!%` is the `{promises}` structure we've seen before.

+ `%...!% (function(e){ rv$res <- NULL ; warning(e) })` is what happens when the `future({})` fails: we're setting the `rv$res` value back to `NULL` so that the `renderPrint()` doesn't fails / print an error in case of failure. 

#### Potential Pitfalls of Asynchronous Shiny

There is one thing to be aware of if you plan on using this asyn methodology: that you're not in a sequential context anymore. 
What that implies is that the first `future()` you will send is not necessary the first you'll get back. 
For example, if you send an SQL request to be run asynchronically, and that each takes between 1 an 10 seconds, if you send 3 request in a row, there is a chance that the first to return will be the last one you have sent. 
The issue here, of course, is that as we treat all the results in the same `reactiveValues()`. 

To handle that, we can adopt two different strategies, depending on what we need to achieve. 
...
In that case, instead of waiting for the very last input, you'll need to build a structure that will receive the output, check if this output is the "next in line", store it if it's not, or return it if it is and see if there is another output in the queue. 
