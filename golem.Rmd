...
When starting a new project with `{golem}`, here on an app called `golex`, you'll be face to this specific structure. 

```{r}
fs::dir_tree("golex")
```

Let's focus on these a moment in order to be sure you understand what each one is used to. 
We'll go from to bottom. 

### DESCRIPTION & NAMESPACE

The `DESCRIPTION` and `NAMESPACE` are standard package files (i.e. they are not `{golem}`-specific. 
In the first, you'll find a series of metadata about your package, for example who wrote the package, what's the package version, what is its intent, who to complain to if things go wrong, and also information about external dependencies, the license, the encoding....
This `DESCRIPTION` file will be filled automatically by the first function you'll use from `dev/01_start.R`, and using other functions from the `dev/` scripts. 
The `NAMESPACE` file is one of the most important file of your package. 
...
Explaining how these files are to be filled is out of the scope of this book, as we hope if you're reading this lines you're already familiar with how to build a package. 
...
When you start your app, this folder is prepopulated with three `.R`: `app_server.R`, `app_ui.R`, and `run_app.R`.

#### app_server.R

```{r echo = FALSE}
readLines("golemexample/R/app_server.R") %>%
  glue::as_glue()
```

This first function contains your server logic. 
If you're used to the classic 'ui.R / server.R' methodology, this function is to be a drop in replacement for the function content you've got in your `server.R`. 
...
```{r echo = FALSE}
readLines("golemexample/R/app_ui.R")[1:12] %>%
  glue::as_glue()
```

This piece of the `app_ui.R` is designed to received the counterpart of what you put in your server.
Everything here is to be put after the <code>`r readLines("golemexample/R/app_ui.R")[6]`</code> line. 
...
```{r echo = FALSE}
readLines("golemexample/R/app_ui.R")[13:28] %>%
  glue::as_glue()
```
...
The other part of this function, starting with `tags$head`, creates a `<head>` tag for your appliation. 
...
+ Calling `addResourcePath( 'img', system.file('app/img', package = 'golemexample') )`
+ Adding elements to your UI with `tags$img(src = "img/name.png")`.

#### run_app.R

```{r echo = FALSE}
readLines("golemexample/R/run_app.R") %>%
  glue::as_glue()
```

This `run_app()`function is the one that you'll use to launch the app^[
Very technically speaking, it's the `print()` from the object outputed by `run_app()` that launches the app, but that's another story. 
].
...
The `inst/app/www/` folder contains all the files which are gonna be made available __at application runtime__. 
Any web application has external files that allows it to run^[
Some webpages don't need any external sources, as they don't have any design and are plain HTML, but generally speaking we won't call this format a webapp. 
]. 
...
Be aware that these files are available under the `www/` at __application runtime__, i.e. that the `www/` folder is available by your browser, not by R when it runs / generate your application. 
...
If you want to link to a file that is read during __application generation__, you'll need to do, for example, `includeMarkdown( system.file("app/www/plop.md", package = "golemexample") )`. 
...
As with the `NAMESPACE` and `DESCRIPTION`, these two files are out of scope of this book (and to be honnest, you'll probably never have to interact with them directly). 
