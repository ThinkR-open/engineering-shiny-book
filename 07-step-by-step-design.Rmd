...
Let's stop for a little story: once upon a time, a team of developer was missioned to build an app, and one feature of the app was to do some API requests.  
So far so good, nothing too complicated... until they discovered that the server where the app will be deployed does not have access to the internet, making it impossible to issue API requests from the server.
