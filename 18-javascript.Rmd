...
For example, this simple `{shiny}` code returns a series of HTML tags: 
...
Finally, we will quickly present some of the functions available in `{golem}` that can be used to launch JavaScript.
...
In "Vanilla" JavaScript (i.e without any external plugin installed), you can query these element using methods from the `document` object. 
For example: 
...
You can use this functionality to replace a call to `{shinyalert}`: the result is a little less aesthetically pleasing, but that is easier to implement and maintain.
...
If you want to interact straight from R with NodeJS (JavaScript in the terminal), you can try the `{bubble}` package. 
