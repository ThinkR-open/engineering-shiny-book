...
Before sending it to a remote server or sharing it to the world, **the first step is testing if the package can be installed on your own computer**.
...
If your application is open-source, the package structure also allows to install from GitHub, by using the `remotes::install_github()` function [^deploy-with-golem-1].
...
[^deploy-with-golem-1]: This is also true for any other Version Control system
...
-   RStudio connect, a server-based solution that can deploy `{shiny}` applications and markdown documents (and other kind of content), and that serves them as ordinary websites
...
The second one is meant for [`{shiny}`Proxy](https://www.shinyproxy.io/), an open source solution for deploying containerized `{shiny}` applications, and the third for [Heroku](https://www.heroku.com/), an online service that can serve containerized applications (not specific to `{shiny}`).
