...
Most of the time, issues come from the fact that __there is too much reactivity__, _i.e._ we build apps where too many things happen, and some things are updated way more often than they should, and computations are performed when they should not, and in the end we have a hard time understanding what is really happening inside our application. 
...
server <- function(
  ...
  # Setting a reactiveValues list that will 
  ...
  #  we set its value inside r$date
  observeEvent( input$date , {
    r$date <- input$date
  }, ignoreInit = TRUE)
  ...
  observeEvent( r$date , {
    # We use req() to control the update 
    # of the selectInput(), which 
    # is updated only if the year of the 
    # input is different from the year
    # of the date input
...
One of the reason why this pattern might not be optimal is in the case where your visitors do not have an high speed internet, or when visiting using a smartphone, contexts where every byte counts. 
...
server <- function(
  ...
  observe({
    ...
    # You'll realise that even after the UI
...
On the contrary, if you lighten the data manipulation so that it is done by the back-end, you will have, let's say, one database with 300MB of data, then the database size will remain (more or less constant), and the only RAM used by `{shiny}` will be the data manipulation, not the data storage. 
...
Databases are heavily used in the data science world, and in the software engineering as a whole. 
...
For instance, if the database needs 300mb, and one shiny app 50mb, then 10 apps will be 300mb + 50 * 1b mo. 
