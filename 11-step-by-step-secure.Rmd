...
> "Don't fuck over Future You" `r right_link('JD', 'https://twitter.com/CMastication')`
...
```{r 11-step-by-step-secure-3, eval = TRUE}
...
```{r 11-step-by-step-secure-4, eval = TRUE}
...
```{r 11-step-by-step-secure-5, eval = TRUE}
...
Figure \@ref(fig:gremlins) show an example of running gremlins on the prenoms application.

(ref:gremlinscap) Example of using `gremlins.js` on the "prenoms" `{shiny}` application.

```{r 11-step-by-step-secure-6, echo=FALSE, fig.cap="(ref:gremlinscap)", out.width='100%'}
...
```{r 11-step-by-step-secure-7, eval = FALSE}
# Creating a new test instance
test <- crrry::CrrryOnPage$new(
  # Using the `find_chrome()` function to guess where the 
  # Chrome bin is on our machine
...
# Launch a test, and record a series of snapshots of your application
...
Using this button, you can take various recording of your shiny application at different states, as show in figure \@ref(fig:shinytestcap).

(ref:shinytestcap) General view of a `{shinytest}` window.

```{r 11-step-by-step-secure-9, echo=FALSE, fig.cap="(ref:shinytestcap)", out.width='100%'}
knitr::include_graphics("img/shinytest.png")
```

Then, you can do some changes in your app, and run:

```{r 11-step-by-step-secure-10, eval = FALSE}
shinytest::testApp()
```
...
```{r 11-step-by-step-secure-11, include=FALSE, error=TRUE, eval = TRUE}
...
```{r 11-step-by-step-secure-12 }
library(shinyloadtest)
```

```{r 11-step-by-step-secure-13, eval = FALSE}
...
```{bash 11-step-by-step-secure-15, eval = FALSE}
...
```{r 11-step-by-step-secure-16, eval = FALSE}
# printing the structure of shinylogs
fs::dir_tree("shinylogs", recurse = FALSE)
```

```{r 11-step-by-step-secure-17, echo = FALSE}
...
```{r 11-step-by-step-secure-18, message = FALSE, warning = FALSE}
...
```{r 11-step-by-step-secure-20, fig.cap="(ref:sessionduration)", out.width="100%"}
...
```{r 11-step-by-step-secure-21, eval = FALSE}
# Generating the report
shinyloadtest_report(shinyload_runs)
```

This function will generate an HTML report of all the things computed by `{shinyloadtest}`, as show in figure \@ref(fig:shinyloadtestreport).

(ref:shinyloadtestreport) Webpage generated by `shinyloadtest_report()`

```{r 11-step-by-step-secure-22, echo=FALSE, fig.cap="(ref:shinyloadtestreport)", out.width="100%"}
...
#### B. `{shinyloadtest}`, `{crrry}`, and `{dockerstats}` {-}
...
```{r 11-step-by-step-secure-23, eval = FALSE}
...
```{r 11-step-by-step-secure-25, echo = FALSE, error = TRUE, eval = TRUE, cache=TRUE}
try({system("docker rm hexmake")})
system("docker run --name hexmake --rm -p 2811:80 colinfay/hexmake", wait = FALSE)
Sys.sleep(5)
```

```{r 11-step-by-step-secure-26, eval = FALSE}
# We are launching the docker container using R system() command. 
# Here, we are running the container image called colinfay/hexmake. 
# We are naming the container hexmake using the --name flag, 
# --rm means the container will be removed when stopped, and 
# finally the -p flag defines how to bind the ports of the container
# with the ports of the host (left is the host, right is the container): 
# in other word, here, we bind port 80 of our container to the port 2811
# of our machine.
system(
...
```{r 11-step-by-step-secure-27, eval = TRUE, cache=TRUE}
...
```{bash 11-step-by-step-secure-28, eval = FALSE}
...
```{r 11-step-by-step-secure-29, eval = FALSE}
...
Figure \@ref(fig:dockerstatscap) is what both these processes look side to side.

(ref:dockerstatscap) `{dockerstats}` and `shinycannon` running side-by-side at the same time.

```{r 11-step-by-step-secure-30, echo=FALSE, fig.cap='(ref:dockerstatscap)', out.width='100%'}
...
```{r 11-step-by-step-secure-33, include=FALSE, eval = TRUE, cache=TRUE}
...
```{r 11-step-by-step-secure-34 }
# Launching the container a second time, but name it xmk2 and serve it on port 2708
system(
  "docker run -p 2708:80 --rm --name xmk2 -d colinfay/hexmake", 
  wait = FALSE
)
Sys.sleep(10) # Let the container launch
```


Then, a `{crrry}` job:

```{r 11-step-by-step-secure-35, eval = TRUE, cache=TRUE}
...
```{r 11-step-by-step-secure-36, echo = FALSE}
...
```{r 11-step-by-step-secure-37, fig.cap="(ref:dockerstatsusage)", eval = TRUE, cache=TRUE}
...
Monitoring these changes and how they potentially break our code is an hard task: be it because checking versions of packages on various machines can take time, or because debugging these bugs in your application logs is not straightforward.
...
On that specific case, it turned out that the package version from `{geojsonsf}` [@R-geojsonsf] was `1.2.1` on our development machine, and the one on the `{shiny}` server was updated to `1.3.0`, and there was a breaking change in the package, as shown in figure \@ref(fig:geojsoncap). 
...
```{r 11-step-by-step-secure-38, echo=FALSE, fig.cap='(ref:geojsoncap)', out.width='100%'}
...
> Underlying the philosophy of renv is that any of your existing workflows should just work as they did befo. `r right_link("Introduction to renv", "https://rstudio.github.io/renv/articles/renv.html")`

The first thing to do with `{renv}` is initiating it with the `init()` function. 
```{r 11-step-by-step-secure-39, eval = FALSE}
...
```{r 11-step-by-step-secure-40, comment="", echo = FALSE}
...
Note that these packages may come from CRAN, Bioconductor, GitHub, GitLab, 
...
```{r 11-step-by-step-secure-41 }
# Displaying the strucuture of the folder
...
```{r 11-step-by-step-secure-42, eval = FALSE}
# Installing attempt
...
```{r 11-step-by-step-secure-44, eval = FALSE}
...
```{r 11-step-by-step-secure-45, comment="", echo = FALSE}
...
#### B. Building a Dockerfile for your app {-}
...
```{r 11-step-by-step-secure-46, eval=FALSE}
...
```{r 11-step-by-step-secure-47, echo = FALSE, comment=""}
...
```{r 11-step-by-step-secure-48, comment=""}
...
```{r 11-step-by-step-secure-49, comment=""}
...
```{r 11-step-by-step-secure-50, echo = FALSE, comment=""}
...
```{r 11-step-by-step-secure-51, echo = FALSE, comment=""}
...
```{r 11-step-by-step-secure-52, echo = FALSE, comment=""}
readLines("data-raw/Dockerfile")[17] %>% 
  glue::as_glue()
```
...
#### E. Develop inside a Docker container {-}
