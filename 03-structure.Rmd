...
You probably haven't realized it yet, but if you have built a significant (in term of code base) Shiny application, chances are you have been using a package-like structure without even seeing it. 

Think about your last Shiny application which was created as a single-file (`app.R`) or two files app (`ui.R` and `server.R`), and what is needed there to make it __a production-ready application__.

First of all, you will need to add __metadata__ to your application.
...
As you know, when you want to push your app into production, you do not want to have this conversation with I.T. people: 
...
`*` will lead to a NAMESPACE conflict on the `flatten()` function that can cause you some debugging headache (trust us, we have been there before). 
So, hey, it would be cool if we could have a Shiny app that only imports specific functions from a package, right?

We can not stress it enough that __dependencies matter__. You need to handle them, and handle them correctly if you want to ensure a smooth deployment to production.
...
As with any production-grade software, documentation and testing will help to ensure the usability and sustainability of your app.

#### Documenting your app

Documenting your Shiny app involves explaining features to the end users and also to the future developers (chances are it will be you).
...
This allows to still write the documentation using the same tags as the exported function, without the internal functions being documented in the end package. 
...
Frameworks for package testing are robust and widely documented in the R world, and if you choose to embrace the "Shiny App as a Package" structure, you do not have to put any extra-effort into testing your application back-end: use a canonical testing framework like `{testthat}` [@R-testthat]. 
Learning how to use testing frameworks is not the subject of this chapter, so feel free to refer to the documentation, and see also Chapter 5 of the [workshop: "Building a package that lasts"](https://speakerdeck.com/colinfay/building-a-package-that-lasts-erum-2018-workshop?slide=107). 
...
Modules are one of the most powerful tools for building Shiny Application in a maintainable and sustainable way.
...
A big Shiny application usually requires reusing the pieces of the ui/server scripts, which makes it hard to name and identify similar inputs and outputs.

Accordingly, Shiny requires its outputs and inputs to have a __unique id__. 
...
If you want to learn more about namespaces, please refer to the [7.4 Special environments](https://adv-r.hadley.nz/environments.html#special-environments) chapter from _Advanced R_, or turn to any computer science book, since namespaces are pretty common in any programming language.
...
Shiny modules aim to do three things: simplify "id" namespacing, split the codebase into a series of functions, and allow UI/Server parts of your app to be reused. 
Most of the time, modules are used for the first two aims.
...
No matter how big your application is, it is always a safe to start modularizing from the very beginning. The sooner in the application development process that you begin to modularize, the easier downstream development will be.
...
So, you will be better off building solid foundations for your application from the very beginning.
...
Let's try to transform the above example (the one with two sliders and two action buttons) into a modularized application.
...
One of the hardest parts of using modules is sharing data across them. 
...
This strategy works well, but for large Shiny Apps it might be hard to handle large lists of reactive outputs / inputs and to keep track of how things are organised. 
...
So yes, big files are complex and make development harder.
...
Of course, as with any convention, you might occasionally see fit to deviate from the general pattern. 
