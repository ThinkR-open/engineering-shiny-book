...
In the next chapter you will be introduced to the `{golem}` package, which is __an opinionated framework for building production-ready Shiny Applications__—this framework will be used a lot through this book, and it relies on the idea that every Shiny application should be built as an R package.
...
All this will be explained in details in chapter [Reactivity anti-patterns](#optim-caveat) of this book, and you can find an example of this pattern inside the [{hexmake}](https://github.com/ColinFay/hexmake/blob/master/R/R6.R) application.
...
This is the case for specific computation and algorithm, custom plot or `geom` for `{ggplot2}` [@ggplot22016], specific calls to a database, etc. 
