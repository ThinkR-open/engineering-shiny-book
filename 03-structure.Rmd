...
On top of these files, what so you needed to make it __a production-ready application__, and why is a package the perfect infrastructure?
...
> `{shiny}` modules address the namespacing problem in `{shiny}` UI and server logic, adding a level of abstraction beyond functions `r right_link("Modularizing Shiny app code, <shiny.rstudio.com>", "https://shiny.rstudio.com/articles/modules.html")`
...
```{r 03-structure-1-ter, echo=FALSE, fig.cap="(ref:tidytuesdayappcap)", out.width="100%"}
