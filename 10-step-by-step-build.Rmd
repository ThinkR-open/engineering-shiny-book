...
# Building the app with `{golem}` {#stepbuild}
...
+ Call the `use_package()` function from `{usethis}`: `usethis::use_package("pkg.you.waa git repository for the {hexmake} application
nt.to.add")`
...
+ The `DESCRIPTION` file dictates which packages have to be installed __when your application is installed__. 
...
When building your application, you have to list somewhere these requirements, and the standard way to do that is by using the `DESCRIPTION` file^[Note that most of the time, you will not be filling this by hand, but by using `usethis::use_package()`]. 

+ The `NAMESPACE` file describes how your app interacts with the R session at run time, _i.e_ __when your application is launched__. 
...
This selective import mechanism allows to avoid namespace conflicts: for example between `jsonlite::flatten()` and `purrr::flatten()`
...
> Having a high quality namespace helps encapsulate your package and makes it self-contained. This ensures that other packages won’t interfere with your code, that your code won’t interfere with other packages, and that your package works regardless of the environment in which it’s run. _R Packages_, [@rpkg]
...
More on testing in chapter \@ref(step-secure).
...
# Adding the correct code coverage infrastructure in your application
...
For example, figure \@ref(fig:golemcov) is the output of running the `package_coverage()` function on the `{golem}` package on the 2020-04-29 on the `dev` branch: 

```{r golemcov, echo=FALSE, fig.cap="`{golem}` code coverage results", out.width="100%"}
...
Note that you can also identify files with zero code coverage using the `covr::zero_coverage(covr::package_coverage())` function, which, instead of printing back a metric of coverage for each file, will point to all the lines that are not covered by tests inside your package, as show in figure \@ref(fig:zerocov):

```{r zerocov, echo=FALSE, fig.cap="`{golem}` files with zero code coverage", out.width="100%"}
...
For example, figure \@ref(fig:codecov-tibble) is the general tendency for the code coverage of the `{tibble}` package over the last 6 months (November 2019 - April 2020): 

```{r codecov-tibble, echo=FALSE, fig.cap="CodeCov.io results for the `{tibble}` package", out.width="100%"}
