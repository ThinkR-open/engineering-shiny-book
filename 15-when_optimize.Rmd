...
You can also rebuild data manipulation code use one package to another^[
A common case is rewriting `{dplyr}` to `{data.table}`.
], use complex data structures to optimizing memory usage, etc, etc.
...
For example, the report from `{hexmake}` suggests to "Eliminate render-blocking resources", and most of them are not controlled by the shiny developer: they come bundled with `shiny::fluidPage` itself.
