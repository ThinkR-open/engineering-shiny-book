...
> Adapted from [WikiMedia](https://commons.wikimedia.org/wiki/File:Effects_of_a_bottleneck_on_virus_populations.png)

You can make the circles travel to the bottleneck as fast as you want, the circles will still be slowed by the narrow bottleneck. 
So focus on making the bottlenck larger, before focusing on making the circle travel fast. 
...
There are thousands of tools available to do exactly that, and going through all of them would probably not make any sense. 
So, let's focus getting started with a basic but powerful tool, that comes bundle inside your browser: Google Lighthouse, one of the famous tool for profiling web application, which is bundled into recent versions of Google Chrome. 
The nice thing is that this tool not only cover what you see, but also audits your app with various configuration, notably on mobile and with low bandwidth and/or mimic 3G connection, and can perform it on locally run applications.
Each result from the audit comes with a series of advices and changes you can make on your app to make it better for each indicator.
And of course, last but not least, you also got the results of the metrics you've "passed", and it's always a good mood booster to see our app passing some audited points!

Here is a quick introduction to this tool: 

+ Open Chrome in icognito mode, so that the page performance is not influenced by any of the installed extensions
+ Open your developer console, and go to the "Audit" tab. 
You can also install a command line tool with `npm install -g lighthouse`, then run `lighthouse http://urlto.audit`.
+ Click on "Generate Report"

![](img/lighthouse-audit.png)

Now, Google Lighthouse is computing a series of analysis about your webpage.

![](img/lighthouse-audit-results.png) 

Now, you have some basic but useful indications about your application:

+ Performance. This metric mostly computes the rendering time of the page: how many time does it take to load the app in full, that is to say how many time it takes from the first byte received to the app fully ready.
With Shiny, you will probably get low performance here, notably due to the fact that `{shiny}` is serving external dependencies that you might not be able to control. 
For example, the report from `{hexmake}` advices to "Eliminate render-blocking resources", and most of them are not controled by you. 
