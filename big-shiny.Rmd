...
<span class = "rigth">Austin Powers</span>
...
> With great complexity comes great responsibilities
...
For example, we'll talk about the design / prototype / build / secure / deploy workflow, which helps you reduce the complexity of implementing and maintain new app features. 
...
A measure that sometime comes in the discussion for assessing complexity is the codebase size. 
It's relatively hard to use this metric, as R is very permissive when it comes to identation and line break, and (unlike JavaScript or CSS), it is generally not minified^[
...
It also depends on your coding style and the packages you're using. For example, tidyverse encourages the use of the pipe (`%>%`)^[
"_`%>%` should always have a space before it, and should usually be followed by a new line._", [tidyverse style guide](https://style.tidyverse.org/pipes.html) 
] 
...
```{r eval = FALSE}
library(cloc)
cloc_cran("shiny", .progress = FALSE)
cloc_cran("attempt", .progress = FALSE)
```
